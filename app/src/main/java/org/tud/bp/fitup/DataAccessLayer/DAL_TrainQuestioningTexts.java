...
public class DAL_TrainQuestioningTexts {
    ...
    public static void getTrainQuestioningTexts(
        ...
        if (praiseOrWarn == 1) {
            ...
            try {
                URL url = new URL(DAL_Utilities.getDatabaseURL(activityTrainQuestioning)  + "Administration/motivationText/neg");
            ...
            try {
                URL url = new URL(DAL_Utilities.getDatabaseURL(activityTrainQuestioning)  + "Administration/motivationText/pos");
                Firebase root = new Firebase(url.toString());
                root.addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(DataSnapshot dataSnapshot) {
