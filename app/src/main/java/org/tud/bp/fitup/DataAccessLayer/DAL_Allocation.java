...
public class DAL_Allocation {
    ...
    public static void getAllocation(final Activity activity) {
        ...
        try {
            URL url = new URL(DAL_Utilities.getDatabaseURL(activity) + "Administration/assignment/");
    ...
    public static void getIntensifier(final Context context) {
        try {
            ...
            URL url = new URL(DAL_Utilities.getDatabaseURL(context) + "Administration/ruleintensifier/");
            Firebase root = new Firebase(url.toString());
            root.addListenerForSingleValueEvent(new ValueEventListener() {
                @Override
                public void onDataChange(DataSnapshot dataSnapshot) {
